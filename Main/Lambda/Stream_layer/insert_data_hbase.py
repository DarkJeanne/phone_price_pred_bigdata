...
HBASE_TABLE_NAME = os.getenv('HBASE_SMARTPHONE_TABLE', 'smartphone') # Use string name here
HBASE_CF_INFO = os.getenv('HBASE_SMARTPHONE_CF', 'info') # Use string for CF name
...
def insert_dataHbase(data):
    ...
    while retries < MAX_RETRIES:
        try:
            ...
            # Use connection with timeout and autoconnect=False
            ...
            # Check if table exists (note: encode table name here for tables() check)
            ...
            # Generate row key based on timestamp
            row_key = datetime.now().strftime('%Y%m%d%H%M%S%f')

            # Prepare data to insert - encode strings to bytes
            data_to_insert = {
